    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        itemsRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                // TODO: Add null pointer check for itemsMap
                for (String key : itemsMap.keySet()) {
                    if (!Helper.isNullOrEmpty(key)) {
                        ...
                        if (itemMap instanceof Map) {
                            ...
                            String uniqueItemID = (String) itemObj.get("ID");
                            ...
                            boolean processing;
                            if(itemObj.get("InProcess") != null) {
                                processing = ((Boolean) itemObj.get("InProcess")).booleanValue();
                            } else {
                                Map<String, Object> inProcess = new HashMap<>();
                                inProcess.put("InProcess", false);
                                itemsRef.child(uniqueItemID).updateChildren(inProcess);
                                processing = false;
                            }
//                            boolean bought;
//                            if(itemObj.get("Bought") != null) {
//                                bought = ((Boolean) itemObj.get("Bought")).booleanValue();
//                            } else {
//                                Map<String, Object> Bought = new HashMap<>();
//                                Bought.put("Bought", false);
//                                itemsRef.child(uniqueItemID).updateChildren(Bought);
//                                bought = false;
//                            }
                            boolean inProcess = processing;
                            Listing item = new Listing(key, name, price, url, category, subCategory, inProcess);

                            //filter the item out of the display6 list if necessary
                            if (doNotFilterOutItem(item) && !processing) {
    ...
    private boolean checkIfProcessing(Map<String, Object> item) {
        if(item.get("Processing") != null && ((Boolean) item.get("Processing")).booleanValue()) {
            return true;
        } else {
            return false;
        }
        ...
        searchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextSubmit(String query) {
                ...
                for (Listing item : unfilteredList) {
                    //boolean InProcess = itemsRef.child(item.getID()).child("InProcess");
                    boolean productInProcess = item.getInProcess();
                    if (doNotFilterOutItem(item) && !productInProcess) {
                        listingsList.add(item);
                    }
                }
                mAdapter.notifyDataSetChanged();
            ...
            public boolean onQueryTextChange(String newText) {
                ...
                for (Listing list : unfilteredList) {
                    ...
                    boolean productInProcess = list.getInProcess();
                    if (!productInProcess && text.contains(newText)) { //adding all items that match the query string to the filtered arraylist
